...
class ProjectsView(QWidget):
    ...
    def __init__(self, service:ProjectService, queries:ProjectQueryService, task_service:TaskExecutionService,parent=None,*,read_only:bool=False)->None:
        super().__init__(parent); self.service,self.queries,self.task_service,self.read_only=service,queries,task_service,read_only; self.theme=get_theme(ApplicationSettings().theme_id); self.current_project_id:UUID|None=None; self.editor=ProjectEditorDialog(service,queries,self); self.editor.project_saved.connect(self._saved)
        root=QVBoxLayout(self); root.setContentsMargins(24,20,24,20); self.stack=QStackedWidget(); root.addWidget(self.stack)
        self.landing=QWidget(); ll=QVBoxLayout(self.landing); header=QHBoxLayout(); header.addWidget(SectionHeading("Projects","Plan and track connected work"),1); self.show_archived=QCheckBox("Show archived"); self.new_project_button=QPushButton("+ New Project"); self.new_project_button.clicked.connect(self.editor.open_for_create); header.addWidget(self.show_archived); header.addWidget(self.new_project_button); ll.addLayout(header)
        ...
        self.detail=QWidget(); dl=QVBoxLayout(self.detail); dh=QHBoxLayout(); back=QPushButton("‹ Projects"); back.clicked.connect(self.show_projects); self.heading=SectionHeading("Project",""); dh.addWidget(back); dh.addWidget(self.heading,1); self.pin=QPushButton(); self.pin.clicked.connect(self._toggle_pin); self.edit_button=QPushButton("Edit"); self.edit_button.clicked.connect(self._edit); self.archive=QPushButton(); self.archive.clicked.connect(self._archive); dh.addWidget(self.pin); dh.addWidget(self.edit_button); dh.addWidget(self.archive); dl.addLayout(dh)
        ...
        list_page=QWidget(); lp=QVBoxLayout(list_page); tools=QHBoxLayout(); tools.addWidget(QLabel("Project tasks")); tools.addStretch(); self.new_task_button=QPushButton("+ New Task"); self.new_task_button.clicked.connect(self._new_task); tools.addWidget(self.new_task_button); lp.addLayout(tools); self.table=QTableWidget(0,7); self.table.setHorizontalHeaderLabels(["Task","Status","Priority","Owner","Start","Due","Progress"]); self.table.setSelectionBehavior(QAbstractItemView.SelectionBehavior.SelectRows); self.table.setEditTriggers(QAbstractItemView.EditTrigger.NoEditTriggers); self.table.cellDoubleClicked.connect(self._activate_task); lp.addWidget(self.table); self.no_tasks=QLabel("No project tasks yet. Add your first task."); self.no_tasks.setAlignment(Qt.AlignmentFlag.AlignCenter); self.no_tasks.setObjectName("mutedText"); lp.addWidget(self.no_tasks)
        self.board=ProjectBoard(task_service,queries,read_only=read_only); self.board.task_activated.connect(self.task_selected); self.board.data_changed.connect(self._board_changed)
        self.gantt=GanttView(queries,None if read_only else task_service); self.gantt.task_activated.connect(self.task_selected); self.gantt.data_changed.connect(self._gantt_changed)
        self.tabs.addTab(self.overview,"Overview"); self.tabs.addTab(list_page,"List"); self.tabs.addTab(self.board,"Board"); self.tabs.addTab(self.gantt,"Gantt"); dl.addWidget(self.tabs); self.stack.addWidget(self.detail)
        for button in (self.new_project_button,self.pin,self.edit_button,self.archive,self.new_task_button): button.setEnabled(not read_only)
        self.refresh()
